...
router.get('/public-register', (req, res) => {
    res.status(405).json({ message: 'Method not allowed. Please use POST for registration.' });
});
...
router.post('/public-register', [
    ...
    try {
        const { email, password, name, address, phone, username } = req.body;
        ...
        const userObj = {
            ...
            name,
            username,
            role: 'user', // Public registrations are always regular users
            address: addressStr,
            phone,
            isActive: true
        };
...
router.put('/users/:id', [verifyToken, admin], async (req, res) => {
    try {
        const { username, email, password, role, isActive ,address } = req.body;
        const user = await User.findById(req.params.id);
